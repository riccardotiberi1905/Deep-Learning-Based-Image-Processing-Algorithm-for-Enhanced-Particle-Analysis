...
def adaptive_opening(processed_predictions, median_px_mm=29.98, radius_big=5):
    new_predictions = []
    ...
    for pred in processed_predictions:
        ...
        # Create empty masks for big and small particles
        big_mask = np.zeros_like(mask, dtype=np.uint8)
        mid_mask = np.zeros_like(mask, dtype=np.uint8)
        small_mask = np.zeros_like(mask, dtype=np.uint8)

        # Classify particles
        area_threshold = np.pi * (0.5 ** 2)
        for region in props:
            major_mm = region.major_axis_length / median_px_mm
            area = (region.area / (median_px_mm ** 2))
            if major_mm >= 1 and area>=area_threshold:
                big_mask[labeled_mask == region.label] = 1
            else:
                small_mask[labeled_mask == region.label] = 1

        # Apply morphological opening
        big_mask_opened = opening(big_mask, disk(radius_big))

        # Combine results
        final_mask = (big_mask_opened | small_mask).astype(np.uint8)
        new_predictions.append(final_mask)

    return np.stack(new_predictions)
...
prediction_path = r"C:\Users\ricca\OneDrive\Work\01_Projects\2024\Image Processing Algorithm\Prebuilt NNs\MSR-NET\Ricc\2_classes\recon_predicted.h5"
mask_path = r"C:\Users\ricca\OneDrive\Work\01_Projects\2024\Image Processing Algorithm\Prebuilt NNs\MSR-NET\Ricc\2_classes\recon_target.h5"
...
h5_directory = r"C:\Users\ricca\OneDrive\Work\01_Projects\2024\Image Processing Algorithm\Prebuilt NNs\MSR-NET\Ricc\2_classes\test_1"
...
#2. Apply opening to predicitions
new_predictions = adaptive_opening(processed_predictions, median_px_mm=29.98, radius_big=5)
...
new_predictions_array = remove_70microns_particles(new_predictions, median_px_mm=29.98)
